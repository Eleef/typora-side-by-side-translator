...
- Local plugin settings are the default and store the API key as plaintext in the current user's community plugin data; other programs running as that Windows user can read it. The optional session mode keeps the key only in memory and cannot recover it after closing, restarting, or updating Typora.
