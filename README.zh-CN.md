...
在 Typora 中编辑原始 Markdown，同时在旁边阅读生成的简体中文译文。
...
- **不污染原文目录。** 工作译文和映射文件保存在插件缓存中，只有主动导出时才生成干净的 `文件名.zh.md`。
...
2. 打开社区插件设置，填写 `baseUrl`、仅当前会话保存的 `apiKey`、`model` 和 `timeoutMs`。
3. 打开社区命令面板；在已验证环境中快捷键是 `F2`。
4. 搜索 **Side-by-Side Translator**，执行 **Toggle Pane**。
5. 执行 **Translate Current File**，生成缓存译文。
6. 修改原文后，需要更新时执行 **Refresh Stale Blocks**。
7. 请求运行期间可执行 **Cancel Translation**，停止任务且不覆盖现有缓存。
8. 执行 **Export Target File**，在原文旁生成干净的 `文件名.zh.md`。
...
当前目标语言固定为简体中文。插件支持 OpenAI 兼容远程接口，也可连接仅在本机回环地址监听的模型服务。代码、公式和 HTML 块原样保留；链接文本可以翻译，但网址不会改变。
...
- API key 只保留在当前 Typora 进程内，不由插件持久化保存。
